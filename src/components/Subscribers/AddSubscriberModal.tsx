...
import { AddSubscriberForm, DropdownOption } from '../../types';
...
interface AddSubscriberModalProps {
  ...
  onClose: () => void;
  onSuccess: () => void;
  parashaOptions: DropdownOption[];
  halachaOptions: DropdownOption[];
}
...
export const AddSubscriberModal: React.FC<AddSubscriberModalProps> = ({
  ...
  const [selectedSubscriber, setSelectedSubscriber] = useState<SearchResult | null>(null);
  const [searching, setSearching] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [formData, setFormData] = useState<Partial<AddSubscriberForm>>({
    numberOfWins: 0,
    scholarshipFund: 0,
    parashaAnswersCode: '',
    yiunHalacha: false,
  ...
  const selectSubscriber = (subscriber: SearchResult) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const subscriberData: AddSubscriberForm = {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-90vh overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          ...
          <h2 className="text-xl font-semibold text-gray-900">הוספת מנוי חדש</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex space-x-2 rtl:space-x-reverse">
                ...
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-right"
                  ...
                  {searchResults.map((result, index) => (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-500">{result.yeshiva} - {result.locality}</span>
                        <div>
                          <div className="font-medium">{result.name}</div>
                          <div className="text-sm text-gray-500">{result.code} - {result.idNumber}</div>
                        </div>
                      </div>
                    </button>
                  ))}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <h3 className="font-medium text-blue-900">{selectedSubscriber.name}</h3>
                  <p className="text-sm text-blue-700">{selectedSubscriber.code} - {selectedSubscriber.idNumber}</p>
          ...
          {selectedSubscriber && (
            <div className="space-y-4">
              ...
              <div>
                <label className="flex items-center justify-end space-x-2 rtl:space-x-reverse text-sm font-medium text-gray-700">
                  <span>עיון ההלכה</span>
                  <input
                    type="checkbox"
          ...
          {/* Submit Buttons */}
          <div className="flex justify-start space-x-3 rtl:space-x-reverse pt-6">
            <button
              type="button"
              onClick={onClose}
